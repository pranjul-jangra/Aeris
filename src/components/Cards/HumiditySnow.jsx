    ...
    return (
        <section className={`*:rounded-md grid gap-8 grid-cols-[repeat(auto-fill,minmax(250px,1fr))] *:whitespace-pre-wrap *:shadow-md`}>
            <article className={`relative ${shadow}`}>
                ...
                <p className={`p-3 *:whitespace-pre-wrap flex gap-3 items-center`}>
                    <span className={`font-bold text-xl`}>{obj.day.totalsnow_cm}</span>
                    <span className={`${mutedText}`}>max/cm</span>
                </p>
            </article>
            ...
            <article className={`relative ${shadow}`}>
                ...
                <p className={`p-3 *:whitespace-pre-wrap flex gap-3 items-center`}>
                    <span className={`font-bold text-xl`}>{obj.day.avghumidity}</span>
                    <span className={`${mutedText}`}>avg</span>
                </p>
            </article>
        </section>
    )
