...
export default function TodaysForecast({ theme }) {
    ...
    return (
        <>
            ...
            <section className='flex gap-6 *:shrink-0 mt-3 flex-wrap whitespace-nowrap *:bg-cyan-800/90 *:text-white *:flex *:gap-2 *:items-center *:rounded-md *:py-2 *:px-3 *:shadow-lg *:hover:scale-105 *:hover:shadow-md *:transition-all *:duration-150'>
                {/* Temperature */}
                <div className={`*:flex *:items-center *:gap-2 ${themeShadow}`}>
            ...
            <section className="flex gap-6 mt-6 flex-wrap *:bg-cyan-800/90 *:text-white *:flex *:gap-2 *:items-center *:rounded-md *:py-2 *:px-3 *:shadow-lg *:hover:scale-105 *:hover:shadow-md *:transition-all *:duration-150 *:whitespace-nowrap">
                <p className={`${themeShadow}`}>🌡️ Feels Like: <strong>{weatherData?.current?.feelslike_c} °C</strong></p>
                <p className={`${themeShadow}`}>🔥 Heat Index: <strong>{weatherData?.current?.heatindex_c} °C</strong></p>
                <p className={`${themeShadow}`}>💨❄️ Wind Chill: <strong>{weatherData?.current?.windchill_c} °C</strong></p>
                <p className={`${themeShadow}`}>💧 Dew Point: <strong>{weatherData?.current?.dewpoint_c} °C</strong></p>
                <p className={`${themeShadow}`}>☀️ UV Index: <strong>{weatherData?.current?.uv}</strong></p>
                <p className={`${themeShadow}`}>💨⚠️ Wind Gusts: <strong>{weatherData?.current?.gust_kph} km/h</strong></p>
            </section>

            <section className="flex gap-6 mt-6 flex-wrap *:bg-cyan-800/90 *:text-white *:flex *:gap-2 *:items-center *:rounded-md *:py-2 *:px-3 *:shadow-lg *:hover:scale-105 *:hover:shadow-md *:transition-all *:duration-150">
                {/* Wind */}
                <div className={`${themeShadow}`}>
